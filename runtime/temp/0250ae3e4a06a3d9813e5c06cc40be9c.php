<?php /*a:1:{s:62:"/home/wwwroot/study/application/admin/view/user/user_list.html";i:1562806576;}*/ ?>
<!DOCTYPE html>
<html>
...
<div class="x-body">
        ...
        <form class="layui-form layui-col-md12 x-so">
            ...
            <!--<div class="layui-input-inline">-->
                <!--<select name="type">-->
                    ...
                    <!--<option <?php if(input('get.type') == 1): ?> selected="selected"<?php endif; ?> value="1">真实会员</option>-->
                    <!--<option <?php if(input('get.type') == 2): ?> selected="selected"<?php endif; ?> value="2">模拟会员</option>-->
                <!--</select>-->
            <!--</div>-->
            <input type="text" name="username"  placeholder="会员账号" autocomplete="off" class="layui-input" value="<?php echo input('get.username'); ?>">
            <input type="text" name="nickname"  placeholder="会员名称" autocomplete="off" class="layui-input" value="<?php echo input('get.nickname'); ?>">
            <input type="text" name="pid"  placeholder="邀请人账号" autocomplete="off" class="layui-input" value="<?php echo input('get.pid'); ?>">
            <input type="text" name="agent_name"  placeholder="代理账号" autocomplete="off" class="layui-input" value="<?php echo input('get.agent_name'); ?>">
            <!--<input type="text" name="p_agent"  placeholder="查询代理直属" autocomplete="off" class="layui-input" value="<?php echo input('get.p_agent'); ?>">-->
            <!--<input type="text" name="all_agent"  placeholder="查询代理全部" autocomplete="off" class="layui-input" value="<?php echo input('get.all_agent'); ?>">-->
            <div class="layui-input-inline">
                <select name="status">
                    <option value="">会员状态</option>
                    <option <?php if(input('get.status') == 1): ?> selected="selected"<?php endif; ?> value="1">登录正常</option>
                    <option <?php if(input('get.status') == 2): ?> selected="selected"<?php endif; ?> value="2">登录禁止</option>
                    <option <?php if(input('get.status') == 3): ?> selected="selected"<?php endif; ?> value="3">交易正常</option>
                    <option <?php if(input('get.status') == 4): ?> selected="selected"<?php endif; ?> value="4">交易禁止</option>
                </select>
            </div>
            <div class="layui-input-inline">
            ...
            <a href="<?php echo url('admin/Api/excel_user_list',input('get.')); ?>"  class="layui-btn" style="background-color: #337AB7">导出</a>
        </form>
    </div>
    <xblock>
    ...
    <table class="layui-table">
        ...
        <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <tr>
                <!--<td>-->
                    <!--<div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='<?php echo htmlentities($vo['uid']); ?>'><i class="layui-icon">&#xe605;</i></div>-->
                <!--</td>-->
                <td><?php echo htmlentities($vo['uid']); ?></td>
                <td><?php echo htmlentities($vo['username']); ?></td>
                <td><?php echo htmlentities($vo['nickname']); ?></td>
                <td><?php echo htmlentities(select_user_username($vo['pid'])); ?></td>
                <td><?php echo htmlentities($vo['agent_name']); ?></td>
                <td><?php echo htmlentities(number_format($vo['money'],2,".","")); ?></td>
                <td><?php echo htmlentities(number_format($vo['promise_money'],2,".","")); ?></td>
                <td><?php echo htmlentities($vo['add_time']); ?></td>
                <td class="td-status">
                    <?php if($vo['trade_status'] == 1): ?>
                    <a onclick="trade_status(this,<?php echo htmlentities($vo['uid']); ?>)" href="javascript:;"  title="设置会员禁止交易">
                        <span class="layui-btn layui-btn-normal layui-btn-mini">是</span>
                    </a>
                    <?php else: ?>
                    <a onclick="trade_status(this,<?php echo htmlentities($vo['uid']); ?>)" href="javascript:;"  title="设置会员可以交易">
                ...
                <td class="td-status">
                    ...
                    <a onclick="member_stop(this,<?php echo htmlentities($vo['uid']); ?>)" href="javascript:;"  title="设置会员不可登录">
                        <span class="layui-btn layui-btn-normal layui-btn-mini">是</span>
                    </a>
                    <?php elseif($vo['login_status'] == 2): ?>
                    <a onclick="member_stop(this,<?php echo htmlentities($vo['uid']); ?>)" href="javascript:;"  title="设置会员可登录">
                        <span class="layui-btn layui-btn-danger layui-btn-mini ">否</span>
                    </a>
                    <?php endif; ?>
                </td>
                ...
                <td class="td-status">
                    <a onclick="x_admin_show('会员邀请码','/admin/User/user_code?id=<?php echo htmlentities($vo['uid']); ?>')" href="javascript:;"  title="会员邀请码">
                        <span class="layui-btn layui-btn-mini " style="background-color: #C57979">查看</span>
                    </a>
                </td>
                <td class="td-status">
                    <a onclick="x_admin_show('银行卡信息','/admin/User/bank_list?username=<?php echo htmlentities($vo['username']); ?>&utype=1')" href="javascript:;"  title="银行卡信息">
                ...
                <td class="td-manage">
                    <div class="layui-btn-group">
                        <button class="layui-btn layui-btn-warm layui-btn-sm" onclick="myshow('up',<?php echo htmlentities($vo['uid']); ?>,'<?php echo htmlentities($vo['username']); ?>')">上分</button>
                        <button class="layui-btn layui-btn-sm " style="background-color: #5BB75B;" onclick="myshow('down',<?php echo htmlentities($vo['uid']); ?>,'<?php echo htmlentities($vo['username']); ?>')">下分</button>
                        <button class="layui-btn layui-btn-sm" onclick="x_admin_show('修改','/admin/User/user_edit?uid=<?php echo htmlentities($vo['uid']); ?>')">修改</button>
                        <button class="layui-btn layui-btn-danger layui-btn-sm" onclick="member_del(this,<?php echo htmlentities($vo['uid']); ?>)">删除</button>
                    </div>
                </td>
            </tr>
